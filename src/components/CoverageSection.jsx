            ...
            <div className="max-w-6xl mx-auto px-4 text-center">
                ...
                <div className="md:mt-12 mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    {coverageData.map((item) => (
                        <div key={item.title} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                            <img src={item.img} alt={item.title} className="w-full h-40 object-cover" />
                            <div className="p-4">
                                <h3 className="text-[20px] font-semibold text-(--secondary-color)">{item.title}</h3>
                                <p className="mt-2 text-(--text-color) text-md">{item.description}</p>
                            </div>
                        </div>
                    ))}
                </div>
